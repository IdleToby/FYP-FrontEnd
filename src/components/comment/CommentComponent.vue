<template>
  ...
  <div class="flex mt-2">
    <div class="flex-grow"></div>
    <div class="flex mr-4">
      <el-button v-btn type="primary" icon="Star" :plain="showLike" @click="handleClickLike" round>
        {{ likesNum }}
      </el-button>
    </div>
    <div class="flex">
...
function fetchPublisherInfo() {
  request
    .get('/comment/getCommentPublisher', { params: { publisherId: props.comment.publisherId } })
    .then((res) => {
...
function fetchReplyToInfo() {
  request
    .get('/comment/getCommentPublisher', { params: { publisherId: props.comment.replyToId } })
    .then((res) => {
...
const commentCreateTime = computed(() => {
  ...
  return new Date(props.comment.createTime).toLocaleString('en-MY', options)
})
...
function getLikes() {
  request
    .get('/comment/getCommentLikes', { params: { commentId: props.comment.commentId } })
    .then((res) => {
      likesNum.value = res
    })
    .catch((error) => {
...
function handleInitialLike() {
  request
    .post('/comment/getCommentLikesNum', {
...
function handleClickLike() {
  if (!showLike.value) {
    ...
    request
      .post('/comment/removeCommentLike', {
        commentId: props.comment.commentId,
        publisherId: localUser.value.userId
      })
      // eslint-disable-next-line no-unused-vars
